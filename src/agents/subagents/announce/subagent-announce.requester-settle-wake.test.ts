...
import type { SettledChildOverrides } from "./subagent-announce.requester-settle-wake.test-support.js";
import {
  REQUESTER,
  requesterSettleKey,
  deliverSpy,
...
describe("maybeWakeRequesterAfterAllChildrenSettled", () => {
  ...
  it("retains a yielded wake after a silent final and retries its visible reply", async () => {
    ...
    try {
      ...
      expect(completeBatchSpy).toHaveBeenCalledWith(["run-b"], 1, visible, true);
