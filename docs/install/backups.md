...
4. For Git history: `openclaw backup git restore --repository <dir> --ref
<commit> (--global | --agent <id>) --target <new-database-path>` materializes
   and verifies a fresh database. Move it into place while the Gateway is
   stopped.
5. For Litestream: `litestream restore` writes a fresh database file; move it
   into place the same way.
6. Start the Gateway and check `openclaw health` and `openclaw doctor`.
